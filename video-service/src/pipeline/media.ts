...
async function downloadFromUri(uri: string, destination: string): Promise<void> {
  ...
  if (!response.ok) throw new Error(`Clip download failed (${response.status}): ${uri}`);
...
export async function stageClipFiles(input: {
  ...
  const dir = input.workingDir ?? (await fs.mkdtemp(path.join(os.tmpdir(), "xai-render-")));
...
function buildS3PublicUrl(key: string): string {
  ...
  if (ref) return `https://${ref}.supabase.co/storage/v1/object/public/${env.S3_BUCKET}/${key}`;
  // Generic S3 fallback
