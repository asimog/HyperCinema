...
function buildPostContent(input: {
  ...
  return {
    ...
    content: [
      ...
      "",
      "---",
      `*Generated by MythXEliza on ${new Date().toLocaleDateString()}*`,
      "",
      `#MythX #ElizaOS #AICinema #AutobiographicalVideo`,
    ].join("\n"),
    submolt: "general",
  };
}
...
export async function registerMoltBookAgent(input: {
  ...
  await moltBookAgentStateCollection().doc("mythxeliza").set(
    {
...
async function getStoredAgentCredentials(): Promise<{
  ...
  const doc = await moltBookAgentStateCollection().doc("mythxeliza").get();
...
async function resolveApiKey(): Promise<{
  ...
  if (env.MOLTBOOK_AGENT_API_KEY) {
    return { apiKey: env.MOLTBOOK_AGENT_API_KEY, agentId: "mythxeliza", status: "claimed", claimUrl: null };
  }
