...
const freeGenerateSchema = z.discriminatedUnion("requestKind", [
  ...
  z.object({
    requestKind: z.enum(["generic_cinema", "bedtime_story", "music_video", "scene_recreation"]),
  ...
  try {
    ...
    if (payload.requestKind === "token_video") {
      ...
      const job = await createTokenVideoJob({
        ...
        pricingMode: "private",
        visibility: "private",
        experience: (payload.experience as CinemaExperience) ?? "funcinema",
      ...
      const job = await createPromptVideoJob({
        ...
        sourceEmbedUrl: null,
        sourceMediaProvider: null,
        sourceTranscript: payload.sourceTranscript?.trim() || null,
        stylePreset: payload.stylePreset as VideoStyleId,
        requestedPrompt: payload.requestedPrompt?.trim() || null,
        audioEnabled:
          payload.requestKind === "bedtime_story" || payload.requestKind === "music_video"
            ? true
