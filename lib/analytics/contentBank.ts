...
export const VIDEO_VISUAL_SYMBOLS = [
  "neon trading screens",
  "casino lighting",
  "rocket launches",
  "glowing chart lines",
  "storm clouds over a digital skyline",
  "sunrise after a trading night",
  "group-chat screenshots floating like subtitles",
  "green particles exploding into red static",
];
...
> = {
  opening: {
    ...
    environments: [
      "dark trading room lit by stacked neon charts",
      "digital city skyline covered in floating candlesticks",
      "casino floor made of market dashboards",
    ],
    actions: [
      "trader leans into the first setup as notifications ignite",
      "candles flare on every monitor while tabs multiply",
      "wallet dashboard wakes up like a mission control board",
    ],
    styles: ["hyperreal meme cinema", "fintech noir", "internet-native trailer realism"],
    lighting: ["cold neon blue with chart-green flickers", "screen glow with rising amber highlights"],
    sound: ["low synth rumble, keyboard taps, distant notification pings"],
  },
  rise: {
    shotTypes: ["tracking shot", "medium push-in", "overhead dashboard sweep"],
    cameraMoves: ["parallax drift", "accelerating push-in", "orbiting handheld energy"],
    environments: [
      "chart tunnel with candles pulsing like runway lights",
      "digital arena where tickers streak overhead",
      "war room of glowing terminals and hotkey flashes",
    ],
    actions: [
      "positions stack as the trader chases momentum across screens",
      "captions erupt while candles race upward",
      "wallet rotates between tokens like a roulette wheel",
    ],
    styles: ["kinetic motion-graphic realism", "sports-hype trailer", "high-voltage internet cinema"],
    lighting: ["green pulses fighting red reflections", "flickering neon gradients"],
    sound: ["tense arpeggiated synths, clicks, whooshing HUD transitions"],
  },
  damage: {
    ...
    environments: [
      "screens turning red inside a dim room",
      "storm front over a holographic chart canyon",
      "casino hallway with warning lights strobing",
    ],
    actions: [
      "chart reverses as the trader freezes then re-enters",
      "PnL card fractures into red shards",
      "captions stack faster than the exits",
    ],
    ...
    sound: ["bass drop, distorted notification alarms, muted crowd gasp"],
  },
  pivot: {
    ...
    environments: [
      "split chart world with green on one side and red on the other",
      "digital alley between collapsing and recovering candles",
      "command center with half the screens dark and half relit",
    ],
    actions: [
      "trader chooses between fear and another entry",
      "new setup appears as captions shift tone",
      "wallet pivots into a comeback or deeper chaos",
    ],
    ...
    sound: ["heartbeat kick, rising pads, brief silence before the next move"],
  },
  climax: {
    shotTypes: ["hero shot", "dramatic crane shot", "full-screen scoreboard reveal"],
    cameraMoves: ["rapid vertical climb", "360 orbit", "hard snap zoom into the final candle"],
    environments: [
      "rocket-lit skyline above a chart breakout",
      "cathedral of glowing tick data",
      "market arena exploding in particles and captions",
    ],
    actions: [
      "best or worst trade detonates across the screen",
      "wallet hits peak chaos as the whole room flashes",
      "PnL and token symbols collide into a final set piece",
    ],
    ...
    sound: ["orchestral rise fused with electronic impact, crowd swell, keyboard clatter"],
  },
  aftermath: {
    shotTypes: ["static wide", "slow pull-back", "quiet overhead"],
    cameraMoves: ["gradual pull-out", "drifting lateral fade", "steady lock-off"],
    environments: [
      "empty trading room at sunrise",
      "quiet dashboard with the final PnL card hanging in space",
      "digital skyline after the storm has passed",
    ],
    actions: [
      "trader stares at the final result while captions settle",
      "screens dim and the story closes on the scoreboard",
      "last notification fades as the room goes still",
    ],
    ...
    sound: ["distant synth echo, soft piano, one last keyboard click"],
  },
};
